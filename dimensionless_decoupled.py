...
stop_sim_time = 1000000 #20 + 20*restart
...
#fully coupled 
problem.add_equation("dt(u) - nu*lap(u) + grad(p) + lift(tau_u) = - u@grad(u) + p*grad(logT) + 2*cross(ez,u) - (u@er)*er*tanhr") 
...
problem.add_equation("dt(T) + lift(tau_T) = D*tanhT*lap(T**3 - T - (L**2) * lap(T)) ") 
